...
def test_token_url_uses_auth_host() -> None:
    service = TokenService(config=build_config())
    assert service.token_url == "https://auth.example.com/api/v1/m2m/token"


@mock.patch("src.token.requests.post")
def test_fetch_token_posts_expected_payload(post_mock: mock.Mock) -> None:
    response = mock.Mock()
    ...
    service = TokenService(config=build_config(), scopes=("scope-a", "scope-b"))
...
def test_refresh_token_returns_cached_token_when_valid() -> None:
    service = TokenService(config=build_config())
...
def test_refresh_token_fetches_new_token_when_missing() -> None:
    service = TokenService(config=build_config())
...
def test_refresh_token_fetches_new_token_when_expired() -> None:
    service = TokenService(config=build_config())
    service.token = AccessTokenModel(
        access_token="stale",
        expires_in=10,
